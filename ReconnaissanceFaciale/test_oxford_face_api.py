...
if __name__ == "__main__":
    ...
    res = createPersonGroup(groupId, groupName, "")
    ...
    faceResult = faceDetect(urlImage, None, None)
    persistedFaceId = addPersonFace(groupId, personId, urlImage, None, None)
    print persistedFaceId
    
    resultTrainPersonGroup = trainPersonGroup(groupId)
    
    faceDetectResult = faceDetect('http://www4.pictures.gi.zimbio.com/Cannes+Leonardo+Di+Caprio+Photocall+BHwgyYKvaYil.jpg', None, None)
    ...
    res = getPersonGroupTrainingStatus(groupId)
    res = res.replace('null','None')
    res_dict = eval(res)

    
    listPersons = eval(getListPersonsInPersonGroup(groupId))
    
    person = eval(getPerson(groupId, recognizedPersonId))
