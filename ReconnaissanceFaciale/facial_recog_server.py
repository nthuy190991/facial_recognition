...
def get_face_emotion_api_results(): 
    ...
    if (resp==1):
        age, gender, glasses, emo = call_face_emotion_api(frameFromHTML)
        ...
        simple_message('Attributs faciales', s)
...
"""
==============================================================================
Dialogue using Chrome (and Bluemix) #TODO: put on main thread
==============================================================================
"""
def flask_init():
    ...
    def onStart():
        thread_program = Thread(target = run_program)
...
def retake_validate_photos(step_time, flag_show_photos, imgPath, name):
    ...
    while (b==0):
        ...
        a = yes_or_no('Nouvelles photos', 'Souhaitez-vous réviser vos photos ?', 4)
        if (a==1):
...
"""
==============================================================================
Program body with decision system and redirection
==============================================================================
"""
def run_program():
    
    # Global variables
    global flag_enable_recog, flag_ask, flag_recog, flag_wrong_recog, flag_reidentify, flag_take_photo
    global text, text2, text3
    global tb_nb_times_recog, nom, name0
    global key2
    
    time.sleep(0.25) # Wait for the flask begins
    ...
    #                    print u'Réduire threshold à ' + str(thres)
                ...
                if (key==ord('i') or key==ord('I')):
                    result = call_face_emotion_api(frameFromHTML)
                    key=0
        """
        End of While-loop
        """
...
"""
def quit_program():
    quit_opt = yes_or_no('Exit', 'Voulez-vous vraiment quitter ?', 3)
    cv2.destroyAllWindows()
